...
- [~] T024 Run the full `quickstart.md` validation: seed → demo US1/US2/US3 → run the Playwright spec against a fresh build with `--workers=1`; confirm SC-001…SC-008. **REQUIRES the live local stack — Postgres/Supabase was not reachable in the implement environment (ports 54322/5432 closed), so the Playwright run + seed-driven SC-005/SC-006 checks must be run by a human/CI per `quickstart.md`. The spec is written and typechecks; all env-independent gates (typecheck, lint, build, Vitest) pass.**
