...
export function CustomersClient({ canArchive }: { canArchive: boolean }) {
  ...
  const query = useEntityList<CustomerDto>("customers", { includeArchived });
  ...
  const createMutation = useMutation({
    mutationFn: (input: CreateCustomerInput) => createEntity("customers", input),
    onSuccess: () => {
      setCreateOpen(false);
      setCreateError(null);
  ...
  const archiveMutation = useMutation({
    mutationFn: (id: string) => archiveEntity("customers", id),
    onSuccess: () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <MasterDataTable
        ...
        detailHref={(row) => `/admin/customers/${row.id}`}
