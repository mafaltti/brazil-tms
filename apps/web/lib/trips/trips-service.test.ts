...
describe.skipIf(!hasDb)("trips-service (integration)", () => {
  ...
  // slice 015 reverted slice 014's born-`validated` path: `createTrip` no longer takes an
  // `initialStatus` argument — every trip is born `received` (covered by the create test above). The
  // born-`validated` case was deleted with that parameter.
